          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 lg:gap-6">
            {products.map((product, index) => (
              ...
              >
                ...
                {/* Product Description */}
                <div className="mb-5">
                  <p className="text-claude-medium text-sm leading-relaxed">{product.description}</p>
                </div>

                {/* Product Stats */}
                <div className="flex items-center space-x-5 mb-5 pb-5 border-b border-claude-beige">
                  <div className="flex items-center space-x-1.5">
                    <Users className="w-3.5 h-3.5 text-claude-muted" />
                    <span className="text-xs text-claude-muted">{product.users} {t('common.users')}</span>
                  </div>
                  <div className="flex items-center space-x-1.5">
                    <Star className="w-3.5 h-3.5 text-claude-accent fill-current" />
                    <span className="text-xs text-claude-muted">{product.rating}/5.0</span>
                  </div>
                </div>

                {/* Features List */}
                <div className="mb-6">
                  <h4 className="text-sm font-semibold text-claude-dark mb-3 tracking-tight">{t('common.keyFeatures')}:</h4>
                  <div className="grid grid-cols-1 gap-2">
                    {product.features.map((feature, idx) => (
                      <div key={idx} className="flex items-start">
                        <CheckCircle className="w-4 h-4 text-claude-accent mr-2.5 flex-shrink-0 mt-0.5" />
                        <span className="text-claude-medium text-sm">{feature}</span>
                      </div>
                    ))}
                  </div>
                </div>
